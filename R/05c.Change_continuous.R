Change_continuous=function(ET,SIN,minsample){
  ...
  for(i in internal){
    ...
    cond1=length(left1)>minsample                           # table above does not work if left or right is empty
    cond2=length(right1)>minsample
    cond3=length(left2)>=1
    cond4=length(right2)>=1
